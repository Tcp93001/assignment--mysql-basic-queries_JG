...
INSERT INTO jobs (name, salary, department_id)
  VALUES
  ('Contador',150000.00, 'Contraloria'),
  ('Auditor',45000.00, 'Contraloria'),
  ('Vendedor de alfombras',34000.00, 'Ventas'),
  ('Recolector', 87500.00,'Ventas'),
  ('Aguador',65000.00,'Ventas'),
  ('Programmer', 18000.00,'Sistemas'),
  ('Administrator', 75000.00, 'Sistemas'),
  ('Colleague', 66000.00, 'Sistemas');
...
INSERT INTO employees (first_name, last_name, hire_date,job_id)
  ...
  ('Miguel','Perez','1998-08-29','Aguador'),
  ('Robert','Ford','2013-11-12','Programmer');

#Ejemplo de UPDATE

UPDATE employees
  SET first_name ='Juancho'
    WHERE last_name = 'Scott';

#Ejemplo de DELETE

DELETE FROM employees
  WHERE first_name = 'Miguel' AND last_name = 'Perez';
...
SELECT first_name, last_name, department_id FROM employees
  INNER JOIN jobs ON employees.job_id = jobs.name
...
SELECT job_id FROM employees
  LEFT OUTER JOIN jobs ON jobs.name = employees.job_id
